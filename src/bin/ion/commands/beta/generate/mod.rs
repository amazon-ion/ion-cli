...
use crate::commands::beta::generate::utils::{
    invalid_data_model_error, CodeGenContext, CodeGenResult, DataModel, Field, Import, Language,
};
...
impl IonCliCommand for GenerateCommand {
    ...
    fn run(&self, _command_path: &mut Vec<String>, args: &ArgMatches) -> Result<()> {
        ...
        Self::generate_code(language, schema, output).unwrap();
...
impl GenerateCommand {
    ...
    fn generate_data_model(
        ...
        GenerateCommand::generate_read_api(
            code_gen_context,
            &mut statements,
            &mut context,
            &mut tera_fields,
        )?;
        GenerateCommand::generate_write_api(code_gen_context, &mut context, &mut tera_fields);
    ...
    fn map_constraint_to_data_model(
        ...
        match constraint.constraint() {
            IslConstraintValue::Element(isl_type, _) => {
                GenerateCommand::verify_data_model_consistency(
                    code_gen_context,
                    DataModel::SequenceStruct,
                )?;
    ...
    fn generate_field_value(
        ...
        Ok(match isl_type_ref {
            IslTypeRef::Named(name, _) => {
                if !GenerateCommand::is_built_in_type(name) {
                    imports.push(Import {
                        module_name: name.to_case(Case::Snake),
                        type_name: name.to_case(Case::UpperCamel),
                    });
                }
                let name = GenerateCommand::map_to_base_type(name, &code_gen_context.language);
                GenerateCommand::generate_sequence_field_value(name, code_gen_context)
            }
    ...
    fn generate_read_api(
            ...
            || code_gen_context.data_model == Some(DataModel::SequenceStruct)
        {
            ...
            for tera_field in tera_fields {
                if !GenerateCommand::is_built_in_type(&tera_field.value) {
                    if code_gen_context.data_model == Some(DataModel::SequenceStruct) {
                ...
                while reader.next()? != StreamItem::Nothing {"#
                        ...
                        if !GenerateCommand::is_built_in_type(sequence_type) {
                            statements.push(format!(
