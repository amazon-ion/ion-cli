...
annotations if applicatble) will be displayed. If the value is nested
...
//   an Rc wrapper to allow each of them to own a reference to it.
...
pub fn run(_command_name: &str, matches: &ArgMatches<'static>) {
    match try_run(matches) {
        Ok(_) => {},
        Err(e) => {
            eprintln!("Encountered an unexpected problem: {:?}", e);
            exit(1);
        }
    };
}

fn try_run(matches: &ArgMatches<'static>) -> IonResult<()> {
    // --skip-bytes has a default value, so we can unwrap this safely.
    let skip_bytes_arg = matches
        .value_of("skip-bytes")
        .unwrap();
    let bytes_to_skip = match usize::from_str(skip_bytes_arg) {
        Ok(num_bytes) => num_bytes,
        Err(_) => {
            eprintln!("Invalid value for '--skip-bytes' '{}'", skip_bytes_arg);
            exit(1);
        }
    };
    ...
    let limit_bytes_arg = matches
        ...
        .unwrap();
    let mut limit_bytes = match usize::from_str(limit_bytes_arg) {
        Ok(num_bytes) => num_bytes,
        Err(_) => {
            eprintln!("Invalid value for '--limit-bytes' '{}'", limit_bytes_arg);
            exit(1);
        }
    };
    ...
    // If the user has specified an output file, use it. Otherwise, use STDOUT.
    ...
    if let Some(input_file_iter) = matches.values_of("input") {
        for input_file_name in input_file_iter {
            let mut input_file = match File::open(input_file_name) {
                Ok(file) => file,
                Err(e) => {
                    eprintln!("Could not open '{}': {:?}", input_file_name, e);
                    exit(2);
                }
            };
        ...
        let mut input_file = match tempfile::tempfile() {
            Ok(file) => file,
            Err(e) => {
                eprintln!(
                    "Unable to open temporary file to store STDIN: {:?}.\nTry passing an --input flag instead.",
                    e
                );
                exit(1);
            }
        ...
        io::copy(&mut io::stdin(), &mut writer)
            .expect("Failed to copy STDIN to a temp file.");
        // Get our file handle back from the BufWriter
        input_file = match writer.into_inner() {
            Ok(file) => file,
            Err(e) => {
                eprintln!("Failed to read from temp file containing STDIN data: {:?}", e);
                exit(1);
            }
        };
...
fn inspect_file(input_file_name: &str,
                ...
                output: &OutputRef,
                bytes_to_skip: usize,
                limit_bytes: usize) -> IonResult<()> {
    // mmap involves operating system interactions that inherently place its usage outside of Rust's
    // safety guarantees. If the file is unexpectedly truncated while it's being read,
    // problems could arise.
    let mmap = unsafe {
        match MmapOptions::new().map(&input_file) {
            Ok(mmap) => mmap,
            Err(e) => {
                eprintln!("Could not mmap '{}': {:?}", input_file_name, e);
                exit(2);
            }
        }
    };
    ...
    match ion_data {
        ...
        _ => {
            eprintln!("Input file '{}' does not appear to be binary Ion.", input_file_name);
        }
    };
    Ok(())
}
...
// it just writes a text Ion comment describing the event.
...
impl<'input> IonInspector<'input> {
    fn new(input: &'input [u8], out: OutputRef, bytes_to_skip: usize, limit_bytes: usize) -> IonInspector<'input> {
        ...
        let text_ion_writer = TextWriter::new(Vec::with_capacity(128));
        IonInspector {
            output: out,
            reader,
            bytes_to_skip,
    ...
    // Returns the offset of the first byte that pertains to the value on which the reader is currently
    // parked.
    ...
    fn inspect_level(&mut self) -> IonResult<()> {
        ...
        while let Some((ion_type, _is_null)) = self.reader.next()? {
            ...
            if complete_value_range.end <= self.bytes_to_skip {
                bytes_skipped += complete_value_range.len();
                continue;
            }
            ...
            if bytes_skipped > 0 {
                self.text_buffer.clear();
                write!(&mut self.text_buffer, "// Skipped {} bytes of user-level data", bytes_skipped)?;
                output(
                    &mut self.output,
                    None,
                    None,
...
fn output<T: Display>(output: &mut OutputRef,
                      offset: Option<usize>,
                      length: Option<usize>,
                      indentation: &str,
                      hex_column: &str,
